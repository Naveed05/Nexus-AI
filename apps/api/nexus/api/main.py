...
def create_agent_workflow(
    ...
    try:
        w=agent_orchestrator.create(payload.objective, items, user_id, payload.max_parallel)
...
def create_job(
    ...
    # Keep the browser's BYOK identity aligned with queued execution ownership.
    user_id = (x_nexus_user_id or x_user_id or "local-user").strip() or "local-user"
    _build_task(payload)
    try:
