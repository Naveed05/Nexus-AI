...
from nexus.core.retrieval import RetrievalEngine
...
retrieval_engine = RetrievalEngine(document_workspace)
...
async def upload_dataset(file: UploadFile = File(...)) -> dict:
    ...
    try:
        dataset = dataset_workspace.register(data, filename=filename, file_format=suffix, metadata={"content_type": file.content_type or "application/octet-stream"})
...
def create_workspace(payload: dict) -> dict:
    ...
    try:
        workspace = workspace_registry.create(name=name, owner_id=owner_id, metadata=metadata)
    except ValueError as exc:
...
async def upload_workspace_file(workspace_id: UUID, file: UploadFile = File(...)) -> dict:
    ...
    data = await file.read()
    try:
        file_ref = file_store.put(data, filename=filename, workspace_id=workspace_id, mime_type=file.content_type)
...
async def upload_workspace_document(workspace_id: UUID, file: UploadFile = File(...)) -> dict:
    ...
    try:
        ...
        retrieval_engine.index_document(document.document_id)
...
def list_workspace_documents(workspace_id: UUID) -> list[dict]:
    ...
    return [{"document_id": str(document_id)} for document_id in context.document_ids]
