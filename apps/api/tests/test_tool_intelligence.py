...
def test_selector_maps_data_inspection_to_profile_tool() -> None:
    decision = ToolSelector().select(Task(objective="Inspect this dataset"), PlanStep("inspect_data", "Inspect the dataset"))
...
def test_selector_maps_analysis_to_analysis_tool() -> None:
    decision = ToolSelector().select(Task(objective="Analyze this dataset"), PlanStep("analyze_data", "Analyze this dataset"))
    assert decision.tool is not None
    assert decision.tool.name == "analyze_dataset"


def test_selector_uses_reference_tool_when_dataset_id_is_in_context() -> None:
    decision = ToolSelector().select(Task(objective="Inspect this dataset", context="dataset_id=123"), PlanStep("inspect_data", "Inspect the registered dataset"))
    assert decision.tool is not None
...
def test_selector_returns_no_tool_when_step_has_no_match() -> None:
    decision = ToolSelector().select(Task(objective="Perform an unsupported operation"), PlanStep("unsupported_step", "Perform an unsupported operation"))
    assert decision.tool is None
    assert decision.score == 0.0
    assert "no registered tool" in decision.reasons[0]


def test_selector_keeps_research_synthesis_evidence_first() -> None:
    decision = ToolSelector().select(Task(objective="Research and synthesize evidence"), PlanStep("synthesize", "Synthesize gathered research evidence"))
    assert decision.tool is None
    assert decision.score == 0.0
    assert "no registered tool" in decision.reasons[0]
...
def test_selector_works_with_custom_registry() -> None:
    ...
    registry.register(ToolSpec(name="calculator", description="Calculate arithmetic", input_schema={"type": "object", "properties": {"expression": {"type": "string"}}, "required": ["expression"], "additionalProperties": False}, risk_level="low", handler=calculator))
