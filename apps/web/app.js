...
async function loadApiSettings(){
  try{
    ...
    const selected=data.providers?.find(x=>x.provider===selectedProvider);
    state.providerConfigured=Boolean(selected?.configured || (selectedProvider==="openai" && selected?.server_configured));
    const title=$("provider-status-title"),detail=$("provider-status-detail"),dot=$("provider-dot"),banner=$("setup-banner");
    if(state.providerConfigured){
      if(title)title.textContent=selectedProvider.toUpperCase()+" is connected";
      if(detail)detail.textContent=selected?.configured
        ?"Your "+selectedProvider.toUpperCase()+" key is configured and persisted by the local NEXUS runtime."
        :"NEXUS is using the server-side OpenAI configuration.";
      if(dot)dot.classList.add("connected");
      if(banner)banner.hidden=true;
    }else{
      if(title)title.textContent=selectedProvider.toUpperCase()+" is not connected";
      if(detail)detail.textContent="Add a "+selectedProvider.toUpperCase()+" API key to run AI work.";
