...
function renderJobs(jobs){
  ...
  host.innerHTML=jobs.length?jobs.map(j=>'<article class="job-row"><div class="job-main"><div class="job-title"><span class="job-dot '+esc(j.status)+'"></span><b>'+esc(j.objective)+'</b></div><small>'+esc(j.status)+' · '+j.retries+'/'+j.max_retries+' retries · '+(j.run_id?esc(j.run_id.slice(0,12)):"waiting for run")+'</small></div><div class="job-actions"><span class="status-pill '+(jobTone(j.status)==="danger"?"status-danger":jobTone(j.status)==="warning"?"status-warning":"")+'">'+esc(j.status)+'</span><button class="ghost" data-job-open="'+esc(j.job_id)+'">Open</button></div></article>').join(""):'<div class="empty">No durable jobs yet. Start a goal from Overview.</div>';
  host.querySelectorAll("[data-job-open]").forEach(b=>b.onclick=()=>inspectJob(b.dataset.jobOpen));host.querySelectorAll("[data-job-retry]").forEach(b=>b.onclick=()=>retryJob(b.dataset.jobRetry));
}
...
const commands=[["Overview","overview"],["Workflows","workflows"],["Workspace","workspace"],["Agent center","agents"],["Agent workspace","agent-workspace"],["Jobs","jobs"],["Execution center","runs"],["Knowledge","knowledge"],["Control center","control-center"]];
