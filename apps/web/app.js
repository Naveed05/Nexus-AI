...
async function loadFiles(){const id=state.workspace;if(!id){$("file-list").innerHTML='<div class="empty">Create or select a workspace first.</div>';return}try{const files=await api.get("/api/v1/workspaces/"+id+"/files");$("file-list").innerHTML=files.length?files.map(f=>'<div class="file-item"><div><span>'+esc(f.filename)+'</span>'+(f.dataset_id?'<small class="file-kind">Dataset ready · '+esc(f.dataset_id.slice(0,12))+'…</small>':'<small class="file-kind">Workspace file</small>')+'</div><span class="file-size">'+f.size_bytes+' bytes</span></div>').join(""):'<div class="empty">No files in this workspace.</div>';$("workspace-status").textContent=files.length+" file(s) available as context."}catch(e){$("workspace-status").textContent="Could not load files."}}
...
async function loadWorkspaceIntelligence(){
  ...
  if(!id)return;
  try{
    const data=await api.get("/api/v1/workspaces/"+id+"/intelligence");
    $("wi-files").textContent=data.resource_counts.files;
    $("wi-datasets").textContent=data.resource_counts.datasets;
    $("wi-documents").textContent=data.resource_counts.documents;
    $("wi-ready").textContent=Object.values(data.readiness).filter(Boolean).length+"/"+Object.keys(data.readiness).length;
    $("wi-recommendation").textContent=data.recommendations?.[0]||"Context pack ready";
    $("workspace-intelligence-status").textContent=data.readiness.has_agent_context?"Context ready for execution":"Add resources to enrich this workspace";
  }catch(e){$("workspace-intelligence-status").textContent="Workspace intelligence unavailable."}
}
async function loadKnowledgeHealth(){
  const id=state.workspace;
  if(!id)return;
  try{
    const data=await api.get("/api/v1/workspaces/"+id+"/knowledge/health");
    $("rag-status").textContent=data.documents+" document(s) · "+data.chunks+" chunks · "+(data.embedding_provider||"local");
  }catch(e){$("rag-status").textContent="Knowledge index unavailable."}
}
async function searchWorkspaceKnowledge(){
  const id=state.workspace,q=$("knowledge-query-input").value.trim();
  if(!id||!q){toast("Select a workspace and enter a question","error");return}
  try{
    const data=await api.post("/api/v1/workspaces/"+id+"/knowledge/search",{query:q,top_k:8});
    $("rag-status").textContent=data.result_count+" evidence result(s) · confidence "+Number(data.confidence).toFixed(2);
    $("knowledge-results").innerHTML=data.results.length?data.results.map(r=>'<article class="feature-card"><b>'+esc(r.citation)+'</b><p>'+esc(r.text)+'</p><small>score '+Number(r.score).toFixed(3)+'</small></article>').join(""):'<div class="empty">No grounded evidence found.</div>';
  }catch(e){toast("Knowledge search failed","error")}
}
async function rebuildWorkspaceKnowledge(){
  const id=state.workspace;if(!id)return;
  try{const data=await api.post("/api/v1/workspaces/"+id+"/knowledge/rebuild",{});$("rag-status").textContent=data.indexed_chunks+" chunks indexed";toast("Knowledge index rebuilt")}catch(e){toast("Index rebuild failed","error")}
}
async function loadDataScience(){
  const id=state.workspace;if(!id)return;
  try{
    const files=await api.get("/api/v1/workspaces/"+id+"/files");
    const dataset=files.find(x=>x.dataset_id);
    if(!dataset){$("data-science-summary").textContent="No analysis-ready dataset in this workspace.";return}
    const report=await api.post("/api/v1/datasets/"+encodeURIComponent(dataset.dataset_id)+"/analyze",{});
    $("data-science-summary").textContent=dataset.filename+" · "+report.profile.rows+" rows · "+report.profile.columns+" columns";
    const flags=report.quality.quality_flags||{};
    $("data-science-results").innerHTML='<div class="control-grid"><article><small>Rows</small><strong>'+report.profile.rows+'</strong><span>'+report.profile.columns+' columns</span></article><article><small>Duplicates</small><strong>'+report.profile.duplicate_rows+'</strong><span>quality check</span></article><article><small>Missing</small><strong>'+Object.keys(report.eda.numeric||{}).length+'</strong><span>'+(flags.has_missing_values?"review needed":"no missing flags")+'</span></article><article><small>Problem</small><strong>'+esc(report.problem.type||"descriptive")+'</strong><span>'+esc(report.problem.target||"no target")+'</span></article></div><p class="muted" style="margin-top:10px">'+esc((report.recommendations||[])[0]||"Dataset is ready for exploration.")+'</p>';
  }catch(e){$("data-science-summary").textContent="Dataset analysis unavailable."}
}

async function loadWorkspaceData(){await Promise.all([loadFiles(),loadMemories(),loadWorkspaceIntelligence(),loadKnowledgeHealth(),loadDataScience()]);const id=state.workspace;$("knowledge-summary").textContent=id?"Workspace "+id.slice(0,12)+" · live context":"No workspace selected";$("knowledge-detail").innerHTML=id?'<div class="file-item"><span>Active workspace</span><span class="file-size">'+esc(id)+'</span></div>':'<div class="empty">Select a workspace to inspect its context.</div>'}
